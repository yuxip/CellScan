  ...
  Int_t CmpRateCor_bit( Int_t fillnum = 15419 ,const char* sl = "large" ){
	...
	if(!strcmp(sl,"large")){
		sprintf(outfilename,"largeRateVsGcor_fill%d_n1.ps",fillnum);
		n1 = 1;
		n2 = 1;         //analysis each nstb separately
	}
	else if(!strcmp(sl,"small")){
		sprintf(outfilename,"smallRateVsGcor_fill%d.ps",fillnum);
		n1 = 3;
		n2 = 4;
	}
	else{
	...
	while(inlist>>runnum){
		...
		fcmp[cnt-1] = new TH2F(title,title,100,-1,1,3,0,3);
