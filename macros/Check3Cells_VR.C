...
//Yuxi Pan 02/26/2012
  Int_t Check3Cells_VR(){
	...
	for(Int_t i = 0; i < 3; i++){
		...
		sprintf(cellname[i],"Cellr%d_c%d_n%d",row0[i],col0,nstb);
		...
		t1[i]->Print();
		sprintf(hname[i],"hCellr%d_c%d_n%d",row0[i],col0,nstb);
		sprintf(fname[i],"fCellr%d_c%d_n%d",row0[i],col0,nstb);
