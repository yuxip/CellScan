...
using namespace::std;
  ...
  Int_t CombineSingle( const char* runlist = "/home/yuxip/FMS/CellScan/MgrRun.list" ,const char* ls = "large"){
	...
	for(Int_t nstb = n1; nstb <= n2; nstb++){
		for(Int_t row0 = 0; row0 < 34; row0++){		//fix-up
			for(Int_t col0 = 0; col0 < 17; col0++){
				...
				sprintf(cellfile,"/home/yuxip/FMS/CellScan/cells/psu/%s.root",cn);
...
//				t1->cd();
				...
				while(inlist>>runnum){
					sprintf(infilename,"/home/yuxip/FMS/CellScan/MyCellsPSU_run%d.root",runnum);
